...
from .forms import CustomUserCreationForm, DriverApplicationForm  # Added DriverApplicationForm
from .models import CustomUser, DriverApplication  # Added DriverApplication model
...
def register(request):  
    ...
    return render(request, 'auth_app/register.html', {'form': form, 'errors': form.errors})  # Display form errors
...
def become_driver(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            driver_application = form.save(commit=False)
            driver_application.user = request.user
            driver_application.save()
            return redirect('profile')
    else:
        form = DriverApplicationForm()

    return render(request, 'auth_app/become_driver.html', {'form': form, 'errors': form.errors})  # Display form errors


def logout_view(request):
    logout(request)
    return redirect('login')  # Change 'login' to your actual login page URL name
